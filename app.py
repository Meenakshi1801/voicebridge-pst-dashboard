...
TASK_BANK = {
    ...
    "Pedagogy of Science": [
        ...
        {"Task_ID": "SCI-05", "Task_Category": "Misconception Diagnosis", "Prompt": "A student says that plants take their food directly from the soil. How will you respond?"},
...
if page == "Home":
    ...
    It enables pre-service teachers to respond to subject-specific pedagogical prompts through voice reasoning, 
    short written pedagogical response, and reflective response.

    It also enables the teacher educator or researcher to review submissions, score responses using a rubric,
    examine voice-written alignment, generate diagnostic profiles, analyse task-wise performance, and download scored data.
    """)
...
# -------------------------------------------------------
    ...
    if len(df) == 0:
        ...
        c.metric("Current Score", f"{row['Total_Score']:.0f}/{MAX_SCORE_PER_TASK}")
...
# -------------------------------------------------------
    ...
    if len(df) == 0:
        ...
        name_value = str(student_df["Name"].iloc[0])
        semester_value = str(student_df["Semester"].iloc[0])
        subject_value = str(student_df["Pedagogy_Subject"].iloc[0])

        p1, p2, p3 = st.columns(3)
        p1.metric("Name", name_value if name_value.strip() else "Not provided")
        p2.metric("Semester", semester_value)
        p3.metric("Subject", subject_value)
        ...
        p4.metric("Total Score", f"{total_score:.0f}/{max_possible}")
        p5.metric("Overall %", f"{overall_percentage:.2f}%")
        p6.metric("Tasks Submitted", len(student_df))
...
# -------------------------------------------------------
    ...
    if len(df) == 0:
        ...
        q3.metric("Pedagogy Subjects", df["Pedagogy_Subject"].nunique())
